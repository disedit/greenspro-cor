...
<template>
  <NuxtLink :to="`/events/${event.slug}`" class="group bg-white rounded-xl flex text-secondary">
    <div class="text-center text-base flex flex-col justify-center font-bold leading-none border-r-3 border-primary-soft py-5 px-8">
      <div class="uppercase">{{ month }}</div>
      <div class="text-xl font-bold -my-2">{{ day }}</div>
      <div>{{ year }}</div>
    </div>
    <div class="flex flex-col w-full justify-between gap-1 p-5">
      <h3 class="font-bold text-md leading-tighter text-balance underline decoration-secondary/0 group-hover:decoration-secondary/50 transition-colors duration-300">{{ event.title }}</h3>
