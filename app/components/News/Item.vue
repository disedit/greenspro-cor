...
<template>
  <article class="group flex flex-col bg-white rounded-xl overflow-clip">
    <div class="relative">
      <div class="absolute top-5 left-5 flex flex-wrap gap-1 lg:gap-3 z-2">
    ...
    <div class="p-3 h-full flex">
      <NuxtLink :to="`/news/${post.slug}`" class="flex flex-col gap-10 p-2 text-lg md:text-md text-secondary rounded-xl focus-secondary h-full">
        <h3 class="text-[1.1em] font-bold leading-tight text-balance underline decoration-secondary/0 group-hover:decoration-secondary/50 transition-colors duration-300">
          {{ post.title }}
        </h3>
