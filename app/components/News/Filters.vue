...
const visible = ref(isDesktop.value)

watch(isDesktop, (desktop) => {
  visible.value = desktop
}, { immediate: true })
...
<template>
  <aside class="border-2 border-primary md:border-0 px-4 md:px-0 rounded-xl">
    <button @click="visible = !visible" class="flex md:hidden w-full items-center text-lg font-bold text-primary py-2 gap-1">
      <Icon name="ri:filter-3-line" />
      <h2>Filters</h2>
    </button>
    <Transition name="grow">
      <div v-if="visible" class="2xl:sticky 2xl:top-navbar">
